		...
		<m-skeleton :count="10" :loading="skeletonLoading">
			<view
				...
				<view class="flex-1 w-0 flex flex-col ml-2">
					...
					<text class="text-xs text-info mt-1" selectable>项目名称: {{ item.projectname }}</text>
					<text class="text-xs text-info">计划名称: {{ item.planname }}</text>

					<text class="text-xs text-info">创建时间: {{ item.regtime }}</text>
					<text class="text-xs text-info">负责人: {{ item.charger }}</text>
				</view>
        <view class="absolute bottom-1 right-1">
          <uni-tag v-if="item.state == 0" text="已驳回" size="small" type="error" inverted></uni-tag>
...
export default {
	...
	methods: {
		...
		skipDetailPage(item) {
			uni.navigateTo({
				url: `/pages/issue/detail/detail?id=${item.id}&title=${item.name}&code=${item.code}`
			})
		}
	}
}
