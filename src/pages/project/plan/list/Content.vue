<template>
	<view class="p-2">
		<m-skeleton :count="10" :loading="skeletonLoading">
			...
			>
				<image
					:src="$store.getters.siteUrl + '/' + item.previewimg"
					class="rounded-md"
					mode="aspectFill"
				...
				<view class="flex-1 w-0 flex flex-col ml-2">
					<view class="text-ellipsis w-full">
						<text class="text-sm text-base font-bold">{{ item.name || '无' }}</text>
					</view>
					...
					<text class="text-xs text-info">计划进度: {{ item.progross | percentStr }}</text>
				</view>
				<view class="absolute bottom-1 right-1" @click.stop="skipGanttPage(item)">
					<m-tag text="甘特图" plain size="mini" type="primary" />
				</view>
			</view>
		</m-skeleton>
	...
	methods: {
		async getData() {
			...
			return new Promise(reslove => {
				...
				request
					.get(
						'/?m=api&c=plan&a=plistbyprojectcode',
						{
