				...
				<view class="_tab-item-box" :class="[defaultConfig.itemWidth ? '_clamp' : '_flex']">
					<block v-for="(item, index) in tabList" :key="index">
						<view
							...
							:style="{
								color: modelValue == index ? defaultConfig.activeColor : defaultConfig.color,
...
export default {
	...
	mounted() {
		...
		uni.onWindowResize(this.calcScrollPosition)
	},
	destroyed() {
		uni.offWindowResize(this.calcScrollPosition)
	},
	methods: {
		updateData() {
			let data = []
			...
			query
				...
				.boundingClientRect(res => {
					...
					if (this.modelValue == index) {
						this.tabToIndex(this.modelValue)
					}
