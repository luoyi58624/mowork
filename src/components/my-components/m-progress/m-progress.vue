				...
				<view class="cmd-progress-inner" :style="setCircleStyle">
					...
					<svg viewBox="0 0 100 100" class="cmd-progress-circle">
						...
						<path
							...
							:class="{ 'zero-progress': setProgress === 0 }"
					...
					<!-- 状态文本 start -->
					<block v-if="showInfo">
						<text class="cmd-progress-text" :style="{ color: textColor }" :title="setFormat">
	...
	computed: {
		...
		setCircle() {
			...
			let svgToBase = `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100' class='cmd-progress-circle'%3E%3Cpath d='${
				...
				this.strokeWidth
			}' fill-opacity='0' class='cmd-progress-circle-trail' style='stroke-dasharray: ${len -
				(gapDeg || 0)}px, ${len}px;stroke-dashoffset: -${(gapDeg || 0) /
				2}px;transition: stroke-dashoffset 0.3s ease 0s, stroke-dasharray 0.3s ease 0s, stroke 0.3s;'%3E%3C/path%3E%3Cpath  d='${
				this.setCirclePath
			}' stroke-linecap='${this.strokeShape}' stroke-width='${
				this.strokeWidth
			}' fill-opacity='0' class='cmd-progress-circle-path zero-progress' style='stroke: ${escape(
				currentColor
			)};stroke-dasharray: ${(this.setProgress / 100) *
...
.cmd-progress-circle-path.zero-progress{
